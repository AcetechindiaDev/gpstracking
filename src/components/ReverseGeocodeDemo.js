...
// ✅ Start with Filters screen only
const [showMap, setShowMap] = useState(false);

// ✅ Filters open by default on first load
...
const ensureOptionsNow = useCallback(() => {
  ...
  const optionsSignature = useMemo(
    () => buildOptionsSignatureStable(dropdownBaseVehicles, `${selectedVendor}|${typeFilter}`),
...
if (!cancelled && idx < baseList.length) {
  ...
  }, [optionsSignature, selectedVendor, dropdownBaseVehicles, dropdownVehiclesForRegNo]);
...
>
  ...
  <div className="d-flex align-items-center gap-2">
    ...
    <Button
      ...
      onClick={openFilters}
      style={{ fontSize: 12, fontWeight: 800, padding: "6px 12px" }}
    >
      Filters
    </Button>
  </div>
              ...
              >
                <div
                  style={{
                    ...
                    inset: 0,
                    background:
                      "radial-gradient(900px 260px at 10% 0%, rgba(255,255,255,.26) 0%, rgba(255,255,255,0) 55%)",
                    pointerEvents: "none",
                  }}
                />
                <Card.Body style={{ padding: "18px 20px", position: "relative" }}>
...
>
  ...
  <Card.Body style={{ padding: "10px 14px", position: "relative" }}>
    <div className="d-flex justify-content-between align-items-start">
      <div>
